% EPL master thesis covers template
\documentclass{template/EPL-master-thesis-covers-EN}

% Please fill in the following boxes
% Title of the thesis
\title{Optimization of production planning with resource allocation}

% % Subtitle - remove this line if not applicable
% \subtitle{Optional subtitle}

\usepackage{subfiles}
\usepackage[toc,page]{appendix}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsfonts,amsthm}
\usepackage{bm}
\usepackage{graphicx}
\PassOptionsToPackage{hyphens}{url}
% \usepackage{url}
\usepackage[hidelinks]{hyperref}
\usepackage[figure]{hypcap}
\usepackage{forest}
\usepackage[inline]{enumitem}
\usepackage{flexisym}
\usepackage{adjustbox}
\usepackage{multirow}

\usepackage{lipsum}

\usepackage{float}
\usepackage{listings}
\newfloat{lstfloat}{htbp}{lop}
\floatname{lstfloat}{Listing}
\def\lstfloatautorefname{Listing}

\usepackage{xcolor}
\usepackage{enumitem}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,bending,matrix,positioning}


\usepackage[section]{placeins}



\colorlet{punct}{red!60!black}
\definecolor{background}{HTML}{fbfbfb}
\definecolor{delim}{RGB}{20,105,176}
\definecolor{string}{HTML}{279900}
\colorlet{numb}{magenta!60!black}

\lstset{
    string=[s]{"}{"},
    stringstyle=\color{blue},
    comment=[l]{:},
    commentstyle=\color{black},
}

\lstdefinelanguage{json}{
    basicstyle=\normalfont\ttfamily,
    numbers=left,
    numberstyle=\scriptsize\color{gray},
    stepnumber=1,
    numbersep=8pt,
    showstringspaces=false,
    breaklines=true,
    frame=single,
    string=[s]{"}{"},
    stringstyle=\color{string},
    backgroundcolor=\color{background},
    literate=
     *{0}{{{\color{numb}0}}}{1}
      {1}{{{\color{numb}1}}}{1}
      {2}{{{\color{numb}2}}}{1}
      {3}{{{\color{numb}3}}}{1}
      {4}{{{\color{numb}4}}}{1}
      {5}{{{\color{numb}5}}}{1}
      {6}{{{\color{numb}6}}}{1}
      {7}{{{\color{numb}7}}}{1}
      {8}{{{\color{numb}8}}}{1}
      {9}{{{\color{numb}9}}}{1}
      {:}{{{\color{punct}{:}}}}{1}
      {,}{{{\color{punct}{,}}}}{1}
      {\{}{{{\color{delim}{\{}}}}{1}
      {\}}{{{\color{delim}{\}}}}}{1}
      {[}{{{\color{delim}{[}}}}{1}
      {]}{{{\color{delim}{]}}}}{1},
}


\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}


\lstdefinestyle{scalaStyle}{
  frame=tb,
  language=scala,
  aboveskip=3mm,
  belowskip=3mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle={\small\ttfamily},
  numbers=left,
  numberstyle=\scriptsize\color{gray},
  backgroundcolor=\color{background},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgreen},
  stringstyle=\color{mauve},
  frame=single,
  breaklines=true,
  breakatwhitespace=true,
  tabsize=3,
}




\graphicspath{{images/}{../images/}{../../images}}

% Name of the student author(s)
\author{Florian \textsc{Knop}}

\degreetitle{Master [120] in Computer Science}

% Name of the supervisor(s)
\supervisor{Pierre \textsc{Schaus}}
\secondsupervisor{Charles \textsc{Thomas}}	


% Name of the reader(s)
\readerone{François \textsc{Aubry}}

% Academic year (update if necessary)
\years{2018--2019}

% Document
\begin{document}

  \def\chapterautorefname{Chapter}
  \def\sectionautorefname{Section}
  \def\subsectionautorefname{Subsection}
  \newcommand*{\Appendixautorefname}{Appendix}
  \newcommand{\vone}{Village n\textsuperscript{o}1}

  % Front cover page
  
  \maketitle






  \begin{abstract}
    \normalsize
    This thesis presents a multi-skill staff scheduling problem with resource allocation. 
    We need to assign workers and several resources to jobs which might require various skills while 
    taking into account multiple constraints such as availability.
    We present two models to solve this problem: Constraint Programming and Mixed Integer Programming.
    Comparing their performances shows that Constraint Programming outperforms Mixed Integer Programming in 
    most cases. However, combining the two models offers the best results as we can take advantage 
    of CP to find a good feasible solution quickly and MIP to further optimize this solution.
  \end{abstract}


  \chapter*{Acknowledgements}
  \addcontentsline{toc}{chapter}{\numberline{}Acknowledgements}%

  \subfile{chapters/acknowledgements}
  
  \tableofcontents


  \listoffigures

  \listoftables




  \chapter{Introduction}
  \label{chapter:introduction}
  \subfile{chapters/introduction}


  \chapter{The resource allocation problem}
  \label{chapter:problem}
  \subfile{chapters/problem}


  \chapter{State of the art}
  \label{chapter:sota}

  \subfile{chapters/state-of-the-art/nsp}

  \section{Mixed Integer Programming}
  \subfile{chapters/state-of-the-art/mip}

  \subsection{Gurobi Optimizer}
  \label{subsection:gurobi}
  \subfile{chapters/state-of-the-art/gurobi}

  \section{Constraint Programming}
  \subfile{chapters/state-of-the-art/cp}

  \subsection{OscaR}
  \label{subsection:oscar}
  \subfile{chapters/state-of-the-art/oscar}
  
  \chapter{Models for the resource allocation problem}
  \label{chapter:models}

  \subfile{chapters/models/intro}
  % \subfile{chapters/models/notations}
  \subfile{chapters/models/mip}
  \subfile{chapters/models/cp}

  \chapter{Development and implementation}
  \label{chapter:implementation}
  \subfile{chapters/implementation}


  \chapter{Experiments}
  \label{chapter:experiments}
  
  \subfile{chapters/experiments/experiments}

  \chapter{Conclusion}
  \label{chapter:conclusion}

  \subfile{chapters/conclusion}

  
  \bibliography{
    bibliography/oscar, 
    bibliography/gurobi,
    bibliography/cp-definition,
    bibliography/nsp,
    bibliography/constraints,
    bibliography/json,
    bibliography/others
  } 
  \bibliographystyle{ieeetr}

  \begin{appendices}

  \subfile{chapters/appendix/notations.tex}
  \subfile{chapters/appendix/mip.tex}
  \subfile{chapters/appendix/cp.tex}

  \end{appendices}

  % Back cover page
  \backcoverpage

\end{document}
