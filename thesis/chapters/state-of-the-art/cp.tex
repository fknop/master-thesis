...
of the constraint. The size of this set is known as the arity of the constraint. The number of constraints associated with 
a variable is called the degree of the variable.
A solution to a \texttt{CSP} is an assignment of values $a_i \in D_i$ to $x_i$, that satisfies all the constraints. \cite{cp-definition}
...
\begin{enumerate}
  ...
  \item Relax last best solution: we fix some variables to the last value in the best solutions. This is the 
        step that can change the most for different types of problems. Most of the time, a simple random relaxation is used (i.e. randomly fix a percentage of variables).
  \item Restart.
\end{enumerate}

The entire search can be limited with a time limit, number of solutions or number of restarts. Each independent search 
