...
For each incompatibility, at each demand, two workers cannot have both their variables assigned to one. 
We define that the sum over all the positions of the two incompatible workers must be lower than two.

\begin{align}
  \sum_{l \in P_{d_k}} w_{ajkl} + w_{bjkl} < 2, &&  \forall {(a, b) \in I_{w_1w_2}}, j \in T, k \in D &\label{wc5}
\end{align}
