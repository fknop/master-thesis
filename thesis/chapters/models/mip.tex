...
To represent our problem in \texttt{MIP}, we need three types of variables, one per resource and an additional type of variable for the dummy workers.
...
given position. In \texttt{MIP}, we model this by having the variables $d_{jkl}$, $d$ for \emph{dummy}. This variable is one, if and only if all the 
...
\subsubsection{All positions must be assigned with one worker}
  ...
  \begin{align}
    \sum_{i \in W} w_{ijkl} + d_{jkl} = 1, && \forall k \in D, j \in T_{d_k}, l \in P(d_k) & \label{wc1}
  \end{align}
...
\begin{align}
  \sum_{k \in D} \sum_{l \in P(d_k)} w_{ijkl} \leq 1, && \forall i \in W, j \in T & \label{wc2} 
\end{align}
...
For each incompatibility $\langle i, c\rangle \in I_{wc}$, we set every worker variables of worker $i$, if the client of the 
demand is $c$, to zero.

\begin{align}
  c_{d_k} = c \implies \forall l \ w_{ijkl} = 0, && \forall {\langle i, c\rangle \in I_{wc}}, j \in T, k \in D &\label{wc6}
\end{align}

\subsubsection{Incompatibilities between workers}

For each incompatibility and each demand, two incompatible workers cannot have both their variables assigned to one. 
We define that the sum over all the positions of the two incompatible workers must be lower than two.

\begin{align}
  \sum_{l \in P(d_k)} w_{ajkl} + w_{bjkl} < 2, &&  \forall {\langle a, b\rangle \in I_{w}}, j \in T, k \in D &\label{wc5}
\end{align}
...
$W_{s_{d_k,l}}$ defines the set of workers that satisfy the skill of demand $k$ at position $l$. 

\begin{align}
  w_{ijkl} = 0, && \forall j \in T, k \in D, l \in P(d_k), i \in W \setminus W_{s_{d_k,l}}  \label{wc6}
\end{align}

\subsubsection{Additional skills must be satisfied}

An additional skill must be satisfied by at least one worker in a team. For each additional skill, we sum 
over all the workers that satisfy the skill to check if at least one is working in the team.

\begin{align}
  \sum_{i \in W_s} \sum_{l \in P(d_k)} w_{ijkl} \geq 1, && \forall j \in T, k \in D, s \in S^{+}_{d_k} & \label{wc8}
\end{align}
...
\begin{align}
  l_{ij} + l_{ik} \leq 1, && \forall j \in D, k \in O(d_j), i \in L \label{lc3}
\end{align}
...
In the set of machines $M$, we can have multiple machines equal to the same value $v$.
We define $M_v \subseteq M$ the set of all machines in $M$ equal to $v$.

\begin{align}
  \sum_{i \in M_v} m_{ij} = | \{ m \mid m \in M_{d_j}, m = v \} |, && j \in D, v \in M_{d_j} \label{mc2}
\end{align}
...
\begin{align}
  m_{ij} + m_{ik} \leq 1, && \forall j \in D, k \in O(d_j), i \in M  
\end{align}
...
\begin{align}
  w_{ijkl} \in \{0, 1\}, && \forall i \in W, j \in T, k \in D, l \in P(d_k) &\label{binary1} \\
  d_{jkl} \in \{0, 1\}, && \forall j \in T, k \in D, l \in P(d_k)& \label{binary2} \\
...
\begin{subequations}
  \label{obj}
  \begin{align}
    \textrm{min} \quad & \delta_0 \sum_{k \in D} \sum_{l \in  P(d_k)} \sum_{i \in W} min(\sum_{j \in T} w_{ijkl}, 1) \label{obj:1} \\ 
      + \ & \delta_1 \sum_{r \in R} \big( max(r_{min} - occ_{r_{w}}, 0) + max(r_{max} - occ_{r_{w}}, 0) \big)  \label{obj:3} \\ 
      + \ &\delta_2 \sum_{j \in T}\sum_{k\in D}\sum_{l \in P(d_k)} d_{jkl} \label{obj:2} \\ 
     \textrm{with} \quad & occ_i = \sum_{j \in T} \sum_{k \in D} \sum_{l \in P(d_k)} w_{ijkl}, \quad \forall i \in W  \nonumber \\ 
                         & \bm{\delta} = \langle\delta_0, \delta_1, \delta_2\rangle \label{penalties:mip} \nonumber
  \end{align}
\end{subequations}
\endgroup
...
\begin{enumerate}
  \item The number of different workers for every position between periods of that demand (\ref{obj:1}), $min(\sum_{j \in T} w_{ijkl}, 1)$ is one if the worker $i$ is working for that position at any time, 0 otherwise. Hence, the sum of those values for all workers is equal to the number of workers for that shift.
